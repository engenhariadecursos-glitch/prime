...
export function HeatmapGrid({ days, last = 91 }: Props) {
  ...
  const getCellColor = (date: string | null): string => {
    ...
    if (d.workoutDone) {
      ...
      return score >= 5 ? colors.green : score >= 3 ? colors.orange : '#3a3a1a';
    }
    const isToday = date === format(new Date(), 'yyyy-MM-dd');
    return isToday ? colors.surface3 : colors.redDim;
  };

  const getTooltip = (date: string | null): string => {
    if (!date) return '';
    const d = days[date];
    if (!d) return date;
    const score = Object.values(d.checkins).filter(Boolean).length;
    return `${date}: ${d.workoutDone ? '✅ Treino' : '—'} ${score}/7`;
  };
  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={styles.legend}>
        ...
        {[colors.surface3, '#3a3a1a', colors.orange, colors.green].map((c, i) => (
